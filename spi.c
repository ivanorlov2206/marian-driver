...
#define SPI_WAIT_FOR_AR(tries) { tries = 10; while (tries>0) { if (readl(marian->iobase + 0x70)==0x80000000) break; msleep(1); tries--; }}
...
int marian_spi_transfer(struct marian_card* marian, uint16_t cs, uint16_t bits_write, uint8_t* data_write, uint16_t bits_read, uint8_t* data_read) {
	...
	snd_printdd(KERN_DEBUG "spi_transfer(.., 0x%04x, %u, [%02x, %02x], %u, ..)\n", cs, bits_write, data_write[0], data_write[1], bits_read);

	SPI_WAIT_FOR_AR(tries);
	if (tries==0) {
		snd_printk(KERN_INFO "marian_spi_transfer: Resetting SPI bus\n");
