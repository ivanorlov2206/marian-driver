...
static uint8_t marian_m2_spi_read(struct marian_card* marian, uint8_t adr) {
	...
	if (0==marian_spi_transfer(marian, 0x02, 8, &adr, 8, &buf_in))
		return buf_in;
...
static int marian_m2_output_frame_mode_put(struct snd_kcontrol *kcontrol,
	...
	struct marian_card *marian = snd_kcontrol_chip(kcontrol);
#ifdef DEBUG
	snd_printk(KERN_INFO "marian_m2_output_frame_mode_put(%d) -> %d\n", kcontrol->private_value, ucontrol->value.enumerated.item[0]);
#endif
...
#ifdef DEBUG
	snd_printk(KERN_INFO "marian_m2_init()\n");
#endif
...
void marian_m2_prepare(struct marian_card* marian) {
	...
	snd_printdd(KERN_INFO "marian_m2_prepare()\n");

	snd_printdd(KERN_INFO "  enabling all channels\n");
...
void marian_m2_proc_status(struct marian_card* marian, struct snd_info_buffer* buffer) {
	...
	switch (marian->clock_source) {
	case 1: snd_iprintf(buffer, "Internal DCO\n"); break;
	case 2: snd_iprintf(buffer, "Sync bus\n"); break;
	case 4: snd_iprintf(buffer, "MADI Input 1\n"); break;
	case 5: snd_iprintf(buffer, "MADI Input 2\n"); break;
	default:
		snd_iprintf(buffer, "UNKNOWN\n"); break;
	}

	snd_iprintf(buffer, "Sample format: %s, %s Endian, %s first\n", (spec->shadow_41 & (1<<M2_INT_FLOAT))?"Float":"Integer", (spec->shadow_41 & (1<<M2_ENDIANNESS))?"Little":"Big", (spec->shadow_41 & (1<<M2_BIT_ORDER))?"LSB":"MSB");
